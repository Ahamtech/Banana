...
void HubAccount::initialize()
{
    ...
    if (!_initialized) {
        ...
        if (_hubCache->accountId() < 0) {
            ...
            if (_supportsCompose) {
                int retVal = _udsUtil->addAccountAction(_accountId, QString("bb.action.COMPOSE"), QString(tr("Add Task")),
                        _cardCompose, QString("application"), _itemComposeIconFilename, _itemMimeType, UDS_PLACEMENT_SIGNATURE);
                if (retVal != 0) {
...
bool HubAccount::addHubItem(qint64 categoryId, QVariantMap &itemMap, QString name, QString subject, qint64 timestamp, QString itemSyncId,  QString itemUserData, QString itemExtendedData, bool notify)
{
    ...
    qDebug()<<"accdid"<<_accountId<<"categoryid"<<categoryId<<"name"<<name<<"subject:"<<subject<<"itemmimetyoe"<<_itemMimeType<<"inreadiconfilename"<<_itemUnreadIconFilename<<"itemread"<<itemRead<<"synchid"<<itemSyncId<<"userdata"<<itemUserData<<"extendeddata"<<itemExtendedData<<"timestamp"<<timestamp<<"contextstate\n"<<itemContextState<<"notify"<<notify;
...
void HubAccount::updateAccount(QString type, QString update){
    _udsUtil->updateAccount(_hubCache->accountId(), _name, _headlessTarget, _iconFilename, _lockedIconFilename, _composeIconFilename, update, true, UDS_ACCOUNT_TYPE_OTHER );
}
